...
- 라이브러리 API, 버전별 동작, 설정 방법이 불확실하면 기억으로 단정하지 않고 공식 문서나 연결된 문서 MCP를 확인한다.
- 확인 대상은 기술 스택 표의 기존 라이브러리뿐 아니라 새로 도입하거나 사용법을 변경하는 외부 패키지·도구 전체를 포함한다.
- Context7 MCP가 연결되어 있으면 해당 라이브러리·도구의 최신 사용법 확인에 우선 사용한다.
- Context7 MCP가 연결되어 있지 않으면 연결되어 있지 않다고 말하고, 공식 문서 확인이나 로컬 코드 기준 판단으로 진행한다.
- 단순 구현에는 외부 문서 조회를 남발하지 않는다. 버전 차이, API 사용법, 설정 변경처럼 틀릴 가능성이 있는 경우에만 사용한다.
